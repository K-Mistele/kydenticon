...
import { hsvToRgb, selectColorFromPalette } from "./colors";
 ...
 *
 * @param idcode - The string to generate an identicon for (hash, email, etc).
 * @param size - The size of the identicon.
 * @param colors - The colors to use for the identicon.
 * @returns The identicon data.
 */
export function generateRawIdenticonTable(
	idcode: string,
	size = 5,
	colors?: string[],
): IdenticonResult {
	...
	const hashbin = BigInt(`0x${h}`).toString(2).padStart(1024, "0");
	...
	if (colors && colors.length > 0) {
		...
		const sat =
			(Number.parseInt(hashbin.slice(8, 16) || "0", 2) / 256) * 55 + 45;
		...
		const val =
			(Number.parseInt(hashbin.slice(16, 24) || "0", 2) / 256) * vrange + vbase;
...
export function generateIdenticonSvg(
	...
	const { table, color } = generateRawIdenticonTable(idcode, size, colors);
 ...
 * @param colors - The colors to use for the identicon.
 */
export function generateIdenticonPng(
	idcode: string,
	size = 5,
	pixelSize = 20,
	padding = 0,
